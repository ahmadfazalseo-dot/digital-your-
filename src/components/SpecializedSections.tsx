...
export function SpecializedIndustrySection({ selectedIndustryId }: SpecializedProps) {
  // Compute different values depending on the industry selected
  const isMedSpa = selectedIndustryId === "med-spa" || selectedIndustryId === "dental-practice";
  const isLaw = selectedIndustryId === "law-firm";
  
  const label = isMedSpa 
    ? "Patient Booking Multiplier" 
    : isLaw 
      ? "Legal Case Value Simulator" 
      : "General Lead Acquisition Simulator";

  const metricLabel = isMedSpa 
    ? "Avg Patient Lifetime Value (LTV)" 
    : isLaw 
      ? "Avg Settlement/Case Value" 
      : "Estimated Customer Lifetime Value";

  const defaultValue = isMedSpa ? 1200 : isLaw ? 8500 : 2500;
  ...
  return (
    <section id="special-industry-section" className="py-20 max-w-7xl mx-auto px-6 select-none border-t border-white/5">
      <div className="max-w-3xl mx-auto text-center mb-12 space-y-3">
        <span className="font-mono text-xs text-yellow-400 uppercase tracking-widest inline-flex items-center gap-1.5">
          <TrendingUp className="w-3.5 h-3.5" />
          {label}
        </span>
        <h2 className="font-display font-medium text-3xl text-white tracking-tight">
          Visualize Your Niche Organic Value
        </h2>
        <p className="text-xs text-zinc-400">
          Play with the metrics to estimate the monthly compounding commercial value generated by driving organic target buyers directly to a fast handcoded funnel.
        </p>
      </div>

      <div className="max-w-3xl mx-auto bento-card-bg p-6 md:p-8 rounded-3xl border border-white/5 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        {/* Slider Controls */}
        <div className="md:col-span-6 space-y-5">
          <div className="space-y-2">
            <div className="flex justify-between font-mono text-[10px]">
              <span className="text-zinc-500 uppercase">Estimated Monthly Traffic</span>
              <strong className="text-zinc-200">{visitorCount.toLocaleString()} Visits</strong>
            </div>
            <input 
              type="range" 
              min="1000" 
              ...
              className="w-full accent-blue-500 bg-zinc-950 rounded-lg cursor-pointer h-1.5"
            />
          </div>

          <div className="space-y-2">
            <div className="flex justify-between font-mono text-[10px]">
              <span className="text-zinc-500 uppercase">Opt-In Conversion Rate</span>
              <strong className="text-zinc-200">{conversionRate}%</strong>
            </div>
            <input 
              type="range" 
              min="1" 
          ...
          <div className="bg-zinc-950 p-4 rounded-xl border border-white/5 space-y-1">
            <div className="font-mono text-[8px] text-zinc-500 uppercase">Preset Metric: {metricLabel}</div>
            <div className="text-zinc-200 font-sans font-bold text-sm">${customerLtv.toLocaleString()} USD</div>
          </div>
        </div>

        {/* Results Matrix Block */}
        <div className="md:col-span-6 bg-zinc-950/80 p-6 rounded-2xl border border-white/10 text-center space-y-4">
          <div className="space-y-1">
            <span className="font-mono text-[10px] text-zinc-500 uppercase">Estimated Live Leads/Mo</span>
            <div className="text-3xl font-display font-medium text-white">{estimatedConversions}</div>
          </div>

          <div className="space-y-1">
            <span className="font-mono text-[10px] text-zinc-500 uppercase">Calculated Niche Value Potential</span>
          ...
          <div className="text-[10px] font-mono text-zinc-500">
            *Based on typical 2.5x speed conversion enhancements of handcoded funnels.
          </div>
        </div>
      </div>
    </section>
...
export function SpecializedDeepDiveSection() {
  return (
    <section id="special-deep-dive-section" className="py-20 max-w-7xl mx-auto px-6 select-none border-t border-white/5">
      <div className="max-w-3xl mx-auto text-center mb-12 space-y-3">
        <span className="font-mono text-xs text-emerald-400 uppercase tracking-widest inline-flex items-center gap-1.5">
          <Layers className="w-3.5 h-3.5 text-blue-500 animate-spin" />
          The Micro-Metric Breakdown
        </span>
        <h2 className="font-display font-medium text-3xl text-white tracking-tight">
          Case Study Infrastructure Speeds
        </h2>
        <p className="text-xs text-zinc-400">
          The exact compile-time and caching performance metrics from our flagship case study configurations.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 max-w-6xl mx-auto font-mono text-[10px]">
        
        <div className="p-5 rounded-2xl bg-zinc-900/40 border border-white/5 space-y-3">
          <span className="text-zinc-500 uppercase block">BUILD MEMORY FOOTPRINT</span>
          <div className="text-white font-sans text-lg font-bold">18.4 KB Flat file</div>
          <p className="text-zinc-500 text-[9px] font-sans leading-relaxed">
            Zero heavy package lockups. Clean compiled HTML structure served directly via CDN assets.
          </p>
        </div>

        <div className="p-5 rounded-2xl bg-zinc-900/40 border border-white/5 space-y-3">
          <span className="text-zinc-500 uppercase block">FIRST INPUT DELAY (FID)</span>
          <div className="text-emerald-400 font-sans text-lg font-bold">14 Milliseconds</div>
          <p className="text-zinc-500 text-[9px] font-sans leading-relaxed">
            Immediate browser interface response upon user interaction, exceeding standard thresholds.
          </p>
        </div>

        <div className="p-5 rounded-2xl bg-zinc-900/40 border border-white/5 space-y-3">
          <span className="text-zinc-500 uppercase block">INTERACTIVE RESPONSE STABILITY</span>
          <div className="text-white font-sans text-lg font-bold">0.00% CLS Shift</div>
          <p className="text-zinc-500 text-[9px] font-sans leading-relaxed">
            Layout elements stay perfectly fixed as asynchronous graphical visual assets render.
          </p>
        </div>

        <div className="p-5 rounded-2xl bg-zinc-900/40 border border-white/5 space-y-3">
          <span className="text-zinc-500 uppercase block">CRAWL SPIDER TIME-OUT GATE</span>
          <div className="text-emerald-400 font-sans text-lg font-bold">100% Crawl rate</div>
          <p className="text-zinc-500 text-[9px] font-sans leading-relaxed">
            Robot crawlers parse all semantic schema strings without encountering any client rendering lags.
          </p>
        </div>
